...
-- Consolidated MySQL 8.0+ database for a fresh installation
-- Generated from the Phase 16 project structure and backend queries.
...
-- Remove these DROP statements if importing into an existing database.
...
CREATE TABLE hospitals (
  ...
  CONSTRAINT fk_hospitals_deleted_by FOREIGN KEY (deleted_by) REFERENCES users(id) ON DELETE SET NULL
) ENGINE=InnoDB;
...
-- =========================================================
-- 4. VACCINES AND SCHEDULE TEMPLATES
-- =========================================================
...
CREATE TABLE appointments (
  ...
  notes TEXT NULL,
  status ENUM('PENDING','CONFIRMED','COMPLETED','CANCELLED','REJECTED','RESCHEDULED') NOT NULL DEFAULT 'PENDING',
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  ...
  active_child_slot_key VARCHAR(160)
    GENERATED ALWAYS AS (
      CASE
        WHEN status IN ('PENDING','CONFIRMED','RESCHEDULED')
...
VALUES
  ('Sunrise Children Hospital', 'Baner Road, Pune', '020-40000001'),
  ('Lotus Mother & Child Hospital', 'Hinjewadi, Pune', '020-40000002'),
  ('City Care Paediatric Centre', 'Wakad, Pune', '020-40000003');
...
-- Users should be created through the registration API so passwords are hashed correctly.
-- After registering test accounts, roles can be assigned for development:
-- UPDATE users SET role='DOCTOR' WHERE email='doctor@example.com';
-- UPDATE users SET role='HOSPITAL_ADMIN' WHERE email='hospitaladmin@example.com';

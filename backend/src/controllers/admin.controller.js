...
export const restoreUser = async (req, res, next) => {
  ...
  try {
    ...
    await connection.query('UPDATE users SET is_active = 1, deleted_at = NULL, deleted_by = NULL WHERE id = ?', [target.id]);
    const updated = { ...target, is_active: true, deleted_at: null, deleted_by: null };
    await writeAuditLog({ ...requestAuditContext(req), action: 'USER_RESTORED', entityType: 'USER', entityId: target.id, oldValues: target, newValues: updated, reason: req.body.reason || null, connection });
    await connection.commit();
    return ok(res, updated, 'User restored successfully');
  } catch (error) { await connection.rollback(); next(error); } finally { connection.release(); }
};
  ...
  try {
    ...
    const [[appointments], [schedules], [immunizations], [growth], [medical]] = await Promise.all([
      ...
      pool.query(`SELECT ir.*, v.name AS vaccine_name, d.name AS doctor_name, h.name AS hospital_name
                  ...
                  WHERE ir.child_id = ? ORDER BY ir.administered_on DESC, ir.id DESC`, [childId]),

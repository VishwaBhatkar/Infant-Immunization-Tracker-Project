...
export const listAppointments = async (req,res,next)=>{try{
  ...
  const [rows]=await pool.query(`SELECT a.*,c.name child_name,h.name hospital_name,d.name doctor_name,p.name parent_name,dr.id review_id,dr.rating review_rating,dr.review_text,dr.is_anonymous review_is_anonymous FROM appointments a JOIN children c ON c.id=a.child_id JOIN hospitals h ON h.id=a.hospital_id JOIN users d ON d.id=a.doctor_id JOIN users p ON p.id=a.parent_id LEFT JOIN doctor_reviews dr ON dr.appointment_id=a.id WHERE ${where.join(' AND ')} ORDER BY a.appointment_date DESC,a.appointment_time DESC`,params);ok(res,rows,'Appointments loaded');
}catch(e){next(e)}};

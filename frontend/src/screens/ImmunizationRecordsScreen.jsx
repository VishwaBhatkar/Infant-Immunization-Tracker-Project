...
export default function ImmunizationRecordsScreen({ canManage = false, title = 'Immunization Records', subtitle = 'Official completed vaccination history.' }) {
    ...
    return (<Screen>
      <FlatList data={records} keyExtractor={(item) => String(item.id)} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void load(); }}/>} ListHeaderComponent={<View>
          ...
          <HealthcareBanner source={require('../../assets/images/real-life/immunization-records.webp')} eyebrow="IMMUNIZATION HISTORY" title="A clear record of completed vaccinations" subtitle="Keep vaccination dates, hospital details, batch information and follow-up doses easy to review." accessibilityLabel="Healthcare professional discussing immunization with a parent and child" />
          {canManage && <Card>

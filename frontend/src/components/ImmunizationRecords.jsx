...
export default function ImmunizationRecords({ canManage = false }) {
    ...
    return (<Screen>
      <FlatList data={records} keyExtractor={(item) => String(item.id)} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void load(); }}/>} ListHeaderComponent={<View>
          <Text style={[styles.title, { color: theme.text }]}>Immunization Records</Text>
          <Text style={{ color: theme.muted, marginBottom: 14 }}>Official completed vaccination history.</Text>
          {canManage && <Card>
